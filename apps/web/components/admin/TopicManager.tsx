...
export function TopicManager({ topics }: { topics: Topic[] }) {
  ...
  return (
    <div className="mx-auto max-w-xl">
      <h1 className="font-headline text-2xl text-ink mb-6">Topics</h1>

      <ul className="mb-8">
        {topics.map((topic) => (
          <li key={topic.id} className="border-b border-hairline py-2 text-ink">
            {topic.title}{" "}
            {topic.shortDescription ? <span className="text-muted text-sm">— {topic.shortDescription}</span> : null}
          </li>
        ))}
      ...
      <form onSubmit={onSubmit}>
        <p className="text-sm text-muted mb-2">Add a topic</p>
        <input
          type="text"
          value={title}
