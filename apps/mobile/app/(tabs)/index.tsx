...
import { getCardSignalTopics, personalizeEdition } from "../../lib/personalization";
...
const progressKey = (editionId: string, version: number) => `titli-edition-progress:${editionId}:v${version}`;

export default function Today() {
  const [activeIndex, setActiveIndex] = useState(0);
  const [initialIndex, setInitialIndex] = useState(0);
  const [progressReady, setProgressReady] = useState(false);
  const [displayEdition, setDisplayEdition] = useState<TodayEdition | null>(null);
  ...
  const activeCardSession = useRef<{ editionKey: string; index: number; startedAt: number } | null>(null);
  const isRestoring = useIsRestoring();
  const selectedTopics = useSelectedTopics();
  const { height: windowHeight } = useWindowDimensions();
  const feedHeight = headerHeight > 0 ? Math.max(0, Math.round(windowHeight - headerHeight)) : 0;

  const { data: edition, isPending, isError, refetch } = useQuery({
    queryKey: ["today-edition"],
    queryFn: () => api.getTodayEdition(),
  });
  const { data: hotStories = [] } = useQuery({
    queryKey: ["hot-stories", "balanced-v1", displayEdition?.id, displayEdition?.version],
  ...
  useEffect(() => {
    ...
    void Promise.all([
      ...
      const rankedEdition = personalizeEdition(edition, selectedTopics, readerProfile);
      const storedEdition = await readEditionOrder(rankedEdition);
      const personalizedEdition = storedEdition
        ?? rankedEdition;
      ...
      activeCardSession.current = { editionKey, index: safeIndex, startedAt: Date.now() };
      ...
      if (cancelled) return;
      preparedEdition.current = editionKey;
      activeCardSession.current = { editionKey, index: 0, startedAt: Date.now() };
  ...
  function onIndexChange(index: number) {
    if (displayEdition) {
      ...
      if (previous?.editionKey === editionKey && previous.index !== index) {
        ...
        if (previousCard && durationMs < 2_000) {
          void recordReaderSignal("fast_skip", getCardSignalTopics(previousCard));
        } else if (previousCard && durationMs >= 8_000) {
          void recordReaderSignal("healthy_dwell", getCardSignalTopics(previousCard));
        }
      }
    ...
    if (displayEdition) {
      void AsyncStorage.setItem(progressKey(displayEdition.id, displayEdition.version), String(index));
    }
  }
    ...
    <SwipeableTabScreen current="today">
      <View className="flex-1 bg-bg">
        <View onLayout={(event) => setHeaderHeight(Math.round(event.nativeEvent.layout.height))}>
          <EditorialHeader
            eyebrow={editionDateLabel(displayEdition?.editionDate)}
            title="TITLI"
            compact
            progress={displayEdition ? {
              current: Math.min(activeIndex + 1, displayEdition.cards.length),
              total: displayEdition.cards.length,
            } : undefined}
          />
        </View>
        <View
          className="min-h-0 overflow-hidden"
          style={headerHeight > 0 ? { height: feedHeight } : { flex: 1 }}
        >
        {isPending || isRestoring || !progressReady ? (
          ...
          <StateMessage
            ...
            onPress={() => refetch()}
          />
        ) : displayEdition && displayEdition.cards.length > 0 && feedHeight > 0 ? (
          <CardStack
...
function StateMessage({
  ...
  return (
    <View className="flex-1 items-center justify-center px-8">
      ...
      <Text className="text-center font-headline text-title leading-[34px] text-ink">{title}</Text>
      <Text className="mt-3 text-center font-body text-body leading-6 text-muted">{body}</Text>
      <Pressable onPress={onPress} className="mt-7 rounded-full bg-red px-6 py-3">
